    ...
    class Event{
        ...
        int order_id;
        int point_id;
        Event(int c_id, string act, int ord, int point);
        bool operator<(const Event& right) const {
            return order_id < right.order_id;
        }
        bool operator==(const Event& right) const {
            return (courier_id == right.courier_id && order_id == right.order_id && 
            action == right.action && point_id == right.point_id);
        }
    };
